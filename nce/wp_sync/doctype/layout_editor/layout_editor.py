...
@frappe.whitelist()
def validate_field_changes(doctype_name, changes):
    """
    Validate field property changes BEFORE applying them
    ...
    Returns:
        Validation result with valid: True/False and errors list
    """
    if not doctype_name:
        return {"valid": False, "errors": ["DocType name is required"]}
    
    if not changes:
        return {"valid": False, "errors": ["No changes provided"]}
    ...
    if isinstance(changes, str):
        try:
            ...
            return {"valid": False, "errors": [f"Invalid changes JSON: {str(e)}"]}
    
    # Get current meta to verify fields exist
    try:
        base_meta = frappe.get_meta(doctype_name)
    except Exception as e:
        return {"valid": False, "errors": [f"Cannot load DocType '{doctype_name}': {str(e)}"]}
    ...
    # Validate each field and its property changes
    for fieldname, properties in changes.items():
        # Check 1: Field exists
        if fieldname not in base_fieldnames:
            errors.append(f"Field '{fieldname}' does not exist in {doctype_name}")
            continue
        
        base_field = base_fieldnames[fieldname]
        
        # Check 2: Validate each property
        for prop, value in properties.items():
            # Check 2a: Cannot change fieldname or fieldtype
            if prop in ["fieldname", "fieldtype"]:
                errors.append(f"Cannot change '{prop}' for field '{fieldname}' (use Customize Form instead)")
                continue
            
            # Check 2b: Validate property value types
            if prop in ["reqd", "hidden", "read_only", "bold", "unique", "allow_bulk_edit", 
                       "allow_in_quick_entry", "allow_on_submit", "in_list_view", 
                       "in_standard_filter", "in_global_search", "report_hide", 
                       "collapsible", "print_hide"]:
                # Boolean properties (should be 0 or 1)
                if value not in [0, 1, True, False, "0", "1"]:
                    errors.append(f"Property '{prop}' for field '{fieldname}' must be 0 or 1 (got: {value})")
            
            elif prop in ["columns", "rows", "length", "precision"]:
                # Integer properties
                try:
                    int_val = int(value)
                    if int_val < 0:
                        errors.append(f"Property '{prop}' for field '{fieldname}' must be >= 0 (got: {value})")
                except (ValueError, TypeError):
                    errors.append(f"Property '{prop}' for field '{fieldname}' must be a number (got: {value})")
            
            # Check 2c: Warning if hiding required field
            if prop == "hidden" and value in [1, True, "1"]:
                if base_field.reqd:
                    warnings.append(f"Warning: Hiding required field '{fieldname}' (may cause validation errors)")
            
            # Check 2d: Warning if making hidden field required
            if prop == "reqd" and value in [1, True, "1"]:
                if base_field.hidden or (fieldname in changes and changes[fieldname].get("hidden") in [1, True, "1"]):
                    errors.append(f"Cannot make hidden field '{fieldname}' required (unhide it first)")
    ...
    if valid:
        ...
        prop_count = sum(len(props) for props in changes.values())
        message = f"✅ Validation passed: {change_count} field(s), {prop_count} property change(s)"
