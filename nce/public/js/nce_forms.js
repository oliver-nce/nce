/**
 * NCE Forms - Global JS for custom DocTypes
 * 
 * Note: doctype_js hook doesn't work for custom:1 DocTypes (Frappe limitation)
 * See: https://github.com/frappe/frappe/issues/16325
 * Solution: Use app_include_js with conditional checks
 */

console.log('[NCE] nce_forms.js loaded');  // DEBUG - remove after confirming

// ============================================
// WP Sync Task Form Handler
// ============================================
frappe.ui.form.on('WP Sync Task', {
    refresh: function(frm) {
        console.log('[NCE] WP Sync Task refresh triggered');  // DEBUG
        
        // Display app version in HTML field
        frappe.call({
            method: 'nce.wp_sync.api.get_app_version',
            callback: function(r) {
                if (r.message && frm.fields_dict.app_version_display) {
                    frm.fields_dict.app_version_display.$wrapper.html(
                        '<div style="padding: 8px 12px; background: #e3f2fd; border-radius: 4px; margin-bottom: 10px; display: inline-block;">' +
                        '<strong>App Version:</strong> ' + r.message.display +
                        '</div>'
                    );
                }
            }
    ...
    load_source_columns: function(frm) {
        frappe.call({
            ...
            args: { table_name: frm.doc.source_table },
            callback: function(r) {
                ...
                let d = new frappe.ui.Dialog({
                    ...
                    primary_action: function() {
                        ...
                        d.$wrapper.find('.field-type-select').each(function() {
                            field_types_map[$(this).data('column')] = $(this).val();
                        });
                        d.hide();
...
// ============================================
// WP Sync Settings Form Handler
// ============================================
frappe.ui.form.on('WP Sync Settings', {
    refresh: function(frm) {
        console.log('[NCE] WP Sync Settings refresh triggered');  // DEBUG
            ...
            frappe.confirm(
                ...
                function() {
                    frappe.call({
                        ...
                        args: { task_names: selected.map(item => item.name) },
                        freeze: true,
